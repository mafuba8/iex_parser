# This program parses IEX exchange market data, given via a pcap files of packets in IEX-TP format,
...
class IEXDBParser:
    """
    Class for parsing PCAP files of IEX-TP packets into a SQLITE file.
    """
    ...
    def parse(self):
        ...
        written into the output SQLite file. Also updates the message type counter.
        """
        print(f'Parsing file {self.input_file} ...')

        # Set up connection to SQLite database.
        db_conn = sqlite3.connect(self.output_db)

        # Create the database tables.
        for message_type in self._MESSAGE_TYPES:
            sqlite_functions.create_table(db_conn, message_type)
        ...
        with gzip.open(self.input_file, 'rb') as stream:
            ...
            while True:
                ...
                # Every 10 million packets, write buffer to their respective database tables and print a status message.
                if self.num_packets % 10_000_000 == 0:
                    for t in self._MESSAGE_TYPES:
                        sqlite_functions.insert_rows(db_conn, t, self._output_buffers[t])
            ...
            # Write remaining buffer to database.
            for t in self._MESSAGE_TYPES:
                sqlite_functions.insert_rows(db_conn, t, self._output_buffers[t])
    ...
    def _parse_iex_payload(self, iex_payload: bytes,
        ...
        if cur_offset != payload_len:
            raise Exception(
                "Invalid parser state: cur_offset after parsing all messages does not match the header.")


    def _parse_iex_message(self, message_payload: bytes, packet_capture_time: int, send_time: int):
        """Parses the given message payload according to the given decoder and writes the decoded information
        as a dictionary into the buffer. Each message starts with a message byte indicating the
        ...
        self._output_buffers[message_type].append(message_dict)
...
if __name__ == '__main__':
    """Main method for parsing single IEX files into a SQLite file."""
    import re
    import sys
    import time
    ...
    if len(sys.argv) < 3:
        print(f'Usage: {sys.argv[0]} input_file.pcap.gz output_db.sqlite')
        exit()
    ...
    parser = IEXDBParser(FILE_INPUT, SQLITE_FILE, decoder)
    parser.parse()
