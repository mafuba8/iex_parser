...
def decode_quote_update_message(message_payload: bytes) -> (int, str):
    """Parses the message payload from a Quote Update Message (message type 'Q').
    Returns the raw_timestamp and the message string.
    """
    ...
    symbol = symbol_raw.decode().strip()
    # Decimal point implied by always having 4 decimal places.
    bid_price = round(bid_price_raw * 1e-4, 2)
    ask_price = round(ask_price_raw * 1e-4, 2)

    # Determine the Quote Update flags.
    q_flags = []
    if quote_update_flags & 0x80:
        q_flags.append('HALTED')  # Symbol is halted, paused or otherwise not available for trading on IEX.
    else:
        q_flags.append('ACTIVE')  # Symbol is active, available for trading.
    if quote_update_flags & 0x40:
        q_flags.append('PRE/POST')  # Pre/Post-Market Session.
    else:
        q_flags.append('REGULAR')  # Regular Market Session.
    q_flags_string = '|'.join(q_flags)

    # Create the message string.
    message_string = f'Q,{symbol},{bid_size},{bid_price},{ask_size},{ask_price},{q_flags_string}'
    return timestamp_raw, message_string
...
#############################
### Decoder function for TOPS
#############################
def decode(message_payload: bytes) -> (int, str, str):
    """Parses the given TOPS1.6 message payload by reading the message type byte from it and choosing
    ...
    match message_type:
        ...
        # Trading Message Formats
        case 'Q':
            raw_timestamp, message_string = decode_quote_update_message(message_payload)
        case 'T':
        ...
        case _:
            raise Exception('Unknown TOPS message type')
